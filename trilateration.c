...
/*
*标签坐标解算函数demo
*接口函数GetLocation(&report, &anchorArray[0], &Range_deca[0]);
*第一个参数report为计算结果
*第二个参数anchorArray为基站坐标3-8个，用户在上位机界面手动输入
*第三个参数Range_deca为距离值，4-8个，通过串口数据解析得到
*/
...
int main()
{
	vec3d anchorArray[MAX_ANC_NUM];
    vec3d report;
    int aid, tid, Range[MAX_ANC_NUM]={-1}, lnum, seq, mask;
    int rangetime;
    char c, type;
    int result = 0; 
	
	
    //以下A0-A7为基站坐标测试数据，实际应用时为上位机用户输入
    //用户输入时，至少输入3个基站坐标
	//A0
	anchorArray[0].x = 0; //anchor0.x uint:m
	anchorArray[0].y = 0; //anchor0.y uint:m
	anchorArray[0].z = 3; //anchor0.z uint:m

	//A1
	anchorArray[1].x = 0; //anchor2.x uint:m
	anchorArray[1].y = 10; //anchor2.y uint:m
	anchorArray[1].z = 3; //anchor2.z uint:m

	//A2
	anchorArray[2].x = 10; //anchor2.x uint:m
	anchorArray[2].y = 10; //anchor2.y uint:m
	anchorArray[2].z = 3; //anchor2.z uint:m

	//A3
	anchorArray[3].x = 10; //anchor2.x uint:m
	anchorArray[3].y = 0; //anchor2.y uint:m
	anchorArray[3].z = 3; //anchor2.z uint:m
    ...
    //以下为造串口模拟数据过程
	Range[0] = 7433;  //tag to A0 distance
	Range[1] = 3905;  //tag to A1 distance
	Range[2] = 8078;  //tag to A2 distance
	Range[3] = -1;    //tag to A3 distance
    Range[4] = -1; 
    Range[5] = 17755; 
    Range[6] = 28377; 
    Range[7] = 27663; 
    printf("mc ff %08x %08x %08x %08x %08x %08x %08x %08x 095f c1 00024c24 a0:0\n",Range[0],Range[1],Range[2],Range[3],Range[4],Range[5],Range[6],Range[7]);
    //printf("mc ff %08x %08x %08x %08x 095f c1 00024c24 a0:0\n",Range[0],Range[1],Range[2],Range[3]);
